...
### 当 source = websearch
执行 {depth_searches} 次 WebSearch，覆盖以下维度：
...
搜索关键词参考：
- `{topic} 2026`、`{topic} comparison`、`{topic} market trends`
- `{topic} expert review`、`{topic} statistics data`

### 当 source = notebooklm
使用 Skill 工具调用 notebooklm skill：
1. 设置笔记本上下文 → 获取所有 source 列表 → 提取全文内容
2. 通过 AI 问答提炼：事实、视觉线索、叙事结构、可量化数据、金句
3. 若 source 数量 < 3，自动用 WebSearch 补充

### 当 source = local-file
1. 用 Read 工具读取 `{local_file}` 的完整内容
2. 分析文件内容，提取核心事实、数据、观点
3. 如信息不足以覆盖 9 个章节，用 WebSearch 补充（最多 {depth_searches} 次）
4. 将本地文件内容与补充搜索结果合并，组织为输出格式

### 当 source = manual
不做自动收集。将用户提供的信息组织为输出格式。

## 输出规范
写入文件：`{output_dir}/research.md`，格式：UTF-8 Markdown
...
**用户指定覆盖**：当 `{visual_strategy}` 不是 `auto` 时，**直接使用用户指定的值**，跳过下方判断规则，在输出中写 `### visual_strategy: {visual_strategy}`（用户已设定）。
