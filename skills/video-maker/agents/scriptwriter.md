...
## Goal
为话题 "{topic}" 撰写一份 {duration} 的 {style} 风格分镜脚本。
画面比例：{aspectRatio}
BGM 文件：{bgm_file}
SFX 开关：{sfx_enabled}
模板推荐 BGM：{template_bgm_track}
...
1. 参数指定：{lut_style}（非 auto/空/none 时直接使用）
2. 调研报告推荐
...
```bash
cd {project_root}/skills/video-maker/remotion-template && pnpm exec tsx src/cli/index.ts scene catalog
```

注意：`{project_root}` 是 Producer 传入的项目根目录绝对路径。
...
### BGM 选择逻辑
1. 如果 `{bgm_file}` 非空 → `bgm_track: custom`
2. 如果 `{template_bgm_track}` 非空 → 使用模板推荐的 track
3. 否则按以下规则匹配：
...
- `{sfx_enabled}` 为 false 时，输出空 sfx_cues 列表
...
你正在进行迭代修复。以下是 Evaluator 的评估反馈：

{evaluator_feedback}

### 修复规则
1. **仅修改** feedback 中 `iteration_fixes` 列出的场景和问题，不要重写整个脚本
2. **对照** `contract_violations`，优先修复 severity=critical 的违规
3. **保持** 未被标记的场景内容不变（包括 narration、visual_prompt）
4. **验证** 修复后仍满足编排规则（无连续 3+ 同类型、时长范围等）
5. 修复完成后，在每个修改过的场景标题旁标注 `[FIXED]`（如 `## Scene 3: 技术原理 [FIXED]`）

### 输出
输出完整的 script.md（包括未修改的场景），写入 `{output_dir}/script.md` 覆盖原文件。
{{/evaluator_feedback}}
